...
import { fetchProductCategories, getIcon, ProductCategory } from "@/data/products";

const GRADIENTS = [
  "from-leaf to-deep",
  "from-deep to-charcoal",
  "from-lime to-leaf",
  "from-leaf to-lime",
  "from-deep to-leaf",
  "from-charcoal to-deep"
];
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
            ...
            return (
              ...
              >
                <div
                  className={`absolute -top-20 -right-20 size-48 rounded-full bg-gradient-to-br ${grad} opacity-10 group-hover:opacity-30 blur-2xl transition`}
                />
                <div
                  className={`relative size-14 rounded-2xl bg-gradient-to-br ${grad} grid place-items-center mb-6 shadow-lg`}
                >
                  <Icon className="size-6 text-white" />
                </div>
                <h3 className="relative text-xl font-bold mb-3">{c.name}</h3>
                <p className="relative text-muted-foreground text-sm leading-relaxed mb-6">
                  {c.description || "Explore our range of premium agricultural solutions."}
                </p>
                <Link
